...
const HeroBanner = ({ data }) => {
  const kicker = data?.kicker || "Chương Trình Giáo Dục Trải Nghiệm";
  const heading = data?.heading || "VIẾT LÊN MỤC TIÊU\nXÂY DỰNG TƯƠNG LAI";
  const subtitle = data?.subtitle || "Truyền cảm hứng mạnh mẽ cho học sinh THPT khám phá điểm mạnh bản thân, rèn luyện tư duy tích cực và học cách thiết lập mục tiêu cuộc sống hiệu quả cùng chuyên gia, diễn giả giàu kinh nghiệm.";
  const duration = data?.duration || "mỗi khối 1 buổi";
  const target = data?.target || "Học sinh THPT";
  const location = data?.location || "Tại trường";
  const bgImage = data?.bgImage || "/assets/images/trai-nghiem/cap-thpt/viet-len-muc-tieu/goals_hero_bg.png";
    ...
    <section className="relative min-h-[85vh] w-full flex items-center justify-center overflow-hidden bg-slate-50 pt-[140px] pb-24 z-1 font-sans">
      ...
      <div className="relative z-1 w-full max-w-5xl px-4 text-center">
        ...
        >
          ...
          >
            <span>
              <i className="ph-bold ph-book-open text-abaBlue text-2xl" />
            </span>
          ...
          <motion.h1
            variants={itemVariants}
            className="fw-semibold text-neutral-700 display2 line-height-96 mb-24 font-heading whitespace-pre-line"
          >
            {heading}
          </motion.h1>
          ...
          >
            ...
            >
              ...
              <span>Thời gian: <strong className="text-slate-800">{duration}</strong></span>
            </motion.div>
            ...
            >
              ...
              <span>Đối tượng: <strong className="text-slate-800">{target}</strong></span>
            </motion.div>
            ...
            >
              ...
              <span>Địa điểm: <strong className="text-slate-700">{location}</strong></span>
            </motion.div>
          </motion.div>
