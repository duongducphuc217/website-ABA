  ...
  const fetchBlogDetails = async () => {
    try {
      ...
      if (data.success) {
        ...
        setTitle(blog.title);
        setSlug(blog.slug);
        setCategory(blog.category || "Trải nghiệm");
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Máy chủ phản lỗi không xác định (Mã trạng thái: ${response.status})`);
      }
  ...
  if (loading) {
    return (
      <div style={styles.loadingContainer}>
        ...
        <p style={{ color: "#94a3b8", marginTop: "16px" }}>Đang tải dữ liệu bài viết...</p>
      </div>
    );
  }
    ...
    <div style={styles.wrapper} className="admin-dashboard-wrapper">
      ...
      <main style={styles.main} className="admin-main">
        <header style={styles.header} className="admin-header">
          <div>
            <h1 style={styles.pageTitle}>Chỉnh sửa bài viết</h1>
            <p style={{ color: "#64748b", margin: "4px 0 0 0" }}>Cập nhật lại bài viết trên hệ thống Website ABA</p>
          </div>
                ...
                <div style={styles.uploadBox}>
                  {image ? (
                    <div style={styles.previewContainer}>
                      <img src={image.startsWith("http") || image.startsWith("/") ? image : `/${image}`} alt="Preview Banner" style={styles.previewImage} />
